...
TEST(UtilTest, testMemstr)
{
    ...
    ASSERT_EQ(serial_library::memstr(str1, sizeof(str1), "de", 2), str1Needle);
    ASSERT_EQ(serial_library::memstr(str1, strlen(str1), "g", 1), str1Needle2);
    ASSERT_EQ(serial_library::memstr(str1, sizeof(str1), "12", 2), nullptr);
    ...
    ASSERT_EQ(serial_library::memstr(str3, sizeof(str3), str3Needle, 2), str3NeedleExpected);
    ASSERT_EQ(serial_library::memstr(str3, 0, str3Needle, 2), nullptr);
}
...
TEST(UtilTest, testConvertFromCString)
{
    ...
    //zero value
    i = serial_library::convertFromCString<int>(zero, 4);
    ASSERT_EQ(i, 0);

    //string with zero length
    i = serial_library::convertFromCString<int>(str, 0);
    ASSERT_EQ(i, 0);

    //interesting
    char exC = 'a';
    char c = serial_library::convertFromCString<char>(&exC, 1);
    ASSERT_EQ(c, exC);
    
    //good test of packing T the same size of string
    i = serial_library::convertFromCString<int>(str, 4);
    ASSERT_EQ(i, 16909060);

    //good test of packing T smaller than string
    i = serial_library::convertFromCString<int>(str, 5);
    ASSERT_EQ(i, 16909060);

    //good test of packing string smaller than T
    i = serial_library::convertFromCString<int>(str, 3);
    ASSERT_EQ(i, 66051);

    //had issues with this
    const char ffTest[2] = {0x30, 0x02};
    i = serial_library::convertFromCString<uint16_t>(ffTest, 2);
    ASSERT_EQ(i, 0x3002);
}
...
TEST(UtilTest, testConvertToCString)
{
    ...
    serial_library::convertToCString<int>(0, str, sizeof(str));
    ASSERT_TRUE(memcmp(str, zeroStr, sizeof(str)) == 0);

    //string with zero length
    memset(str, 0, sizeof(str));
    serial_library::convertToCString<int>(6, str, 0);
    ASSERT_TRUE(memcmp(str, zeroStr, sizeof(str)) == 0);

    //interesting
    char c = 0;
    serial_library::convertToCString<char>('B', &c, 1);
    ASSERT_EQ(c, 'B');

    //good test of unpacking T the same size as string
    memset(str, 0, sizeof(str));
    serial_library::convertToCString<int>(16909060, str, 4);
    ASSERT_TRUE(memcmp(str, refStr, 4) == 0);

    //good test of unpacking T smaller than string
    memset(str, 0, sizeof(str));
    serial_library::convertToCString<int>(16909060, str, 5);
    ASSERT_TRUE(memcmp(str, refStr, 4) == 0);
    ASSERT_EQ(str[4], 0);

    //good test of unpacking T bigger than string
    memset(str, 0, sizeof(str));
    serial_library::convertToCString<int>(16909060, str, 3);
...
TEST_F(Type1SerialProcessorTest, testExtractFieldFromBufferBasic)
{
    ...
    size_t result = serial_library::extractFieldFromBuffer(testMsg, sizeof(testMsg), frameMap[0], FIELD_SYNC, &dst, 1);
    ASSERT_EQ(result, 1u);
    ASSERT_EQ(dst, 'A');

    //1-char extraction of middle value
    result = serial_library::extractFieldFromBuffer(testMsg, sizeof(testMsg), frameMap[0], TYPE_1_FRAME_1_FIELD_1, &dst, 1);
    ASSERT_EQ(result, 1u);
    ASSERT_EQ(dst, 'a');

    //1-char extraction of end value
    result = serial_library::extractFieldFromBuffer(testMsg, sizeof(testMsg), frameMap[0], TYPE_1_FRAME_1_FIELD_3, &dst, 1);
    ASSERT_EQ(result, 1u);
    ASSERT_EQ(dst, 'c');
}
...
TEST_F(Type1SerialProcessorTest, testInsertFieldToBufferBasic)
{
    ...
    serial_library::insertFieldToBuffer(dst, sizeof(dst), frameMap[0], FIELD_SYNC, &c, sizeof(c));
    ASSERT_TRUE(memcmp(dst, "jbcd", 4) == 0);

    //1-char insertion of beginning value
    strcpy(dst, "Abcd");
    serial_library::insertFieldToBuffer(dst, sizeof(dst), frameMap[0], TYPE_1_FRAME_1_FIELD_1, &c, sizeof(c));
    ASSERT_TRUE(memcmp(dst, "Ajcd", 4) == 0);

    //1-char insertion of end value
    strcpy(dst, "Abcd");
    serial_library::insertFieldToBuffer(dst, sizeof(dst), frameMap[0], TYPE_1_FRAME_1_FIELD_3, &c, sizeof(c));
...
TEST(UtilTest, testNormalizeSerialFrame)
{
    ...
    SerialFrame normalizedSyncBeginning = serial_library::normalizeSerialFrame(frameSyncBeginning);
    ...
    SerialFrame normalizedSyncMiddle = serial_library::normalizeSerialFrame(frameSyncMiddle);
    ...
    SerialFrame normalizedSyncEnd = serial_library::normalizeSerialFrame(frameSyncEnd);
