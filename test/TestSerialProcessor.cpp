...
TEST_F(Type2SerialProcessorTest, TestBasicRecvAndSendType2WithShortcuts)
{
    ...
    ASSERT_EQ(processor->getFieldValue<char>(TYPE_2_FIELD_3), 'd');
    ASSERT_EQ(processor->getFieldValue<char>(TYPE_2_FIELD_4), 'E');

    Time sendTime = now, worstCaseRecvTime = serial_library::curtime();
    ASSERT_GT(processor->getFieldTimestamp(TYPE_2_FIELD_3), sendTime);
    ASSERT_GT(processor->getFieldTimestamp(TYPE_2_FIELD_4), sendTime);
    ASSERT_LT(processor->getFieldTimestamp(TYPE_2_FIELD_3), worstCaseRecvTime);
    ASSERT_LT(processor->getFieldTimestamp(TYPE_2_FIELD_4), worstCaseRecvTime);
...
TEST_F(CallbacksTest, TestMsgReceivedCallback)
{
    ...
    serial_library::SerialValuesMap received = lastReceivedSerialFrames();

    SerialDataStamped field = received[TYPE_2_FIELD_1];
    ASSERT_TRUE(memcmp(field.data.data, "1", field.data.numData) == 0);
    field = received[FIELD_FRAME];
    SerialFieldId id = TYPE_2_FRAME_1;
    ASSERT_TRUE(memcmp(field.data.data, &id, field.data.numData) == 0);
    field = received[TYPE_2_FIELD_2];
...
TEST_F(CallbacksTest, TestChecksumFuncs)
{
    ...
    senderProcessor.send(TYPE_2_CHKSM_FRAME);
    senderProcessor.send(TYPE_2_CHKSM_FRAME);
    ...
    Checksum expectedChecksum = 'p' + TYPE_2_CHKSM_FRAME + 'A' + '1' + 'q';
    ASSERT_EQ(lastGeneratedChecksum(), expectedChecksum);

    ASSERT_TRUE(compareSerialData(processor->getField(TYPE_2_FIELD_1).data, serial_library::serialDataFromString("1", 1)));
    ASSERT_TRUE(compareSerialData(processor->getField(TYPE_2_FIELD_5).data, serial_library::serialDataFromString("pq", 2)));
}
...
TEST(GenericType2SerialProcessorTest, TestConstructorFrameValueAssertions)
{
    ...
    //frame with not 0 nor 2 checksum fields
    SerialFramesMap threeChecksums = {
        {Type2SerialFrames1::TYPE_2_FRAME_1,
            {
                FIELD_CHECKSUM,
                FIELD_SYNC,
    ...
    trans = std::make_unique<TestTransceiver>(true);
    ASSERT_THROW(
        serial_library::SerialProcessor proc(std::move(trans), threeChecksums, Type2SerialFrames1::TYPE_2_FRAME_1, "a", 1),
        SerialLibraryException);

    //frame with not 0 nor 2 checksum fields
    SerialFramesMap oneChecksum = {
        {Type2SerialFrames1::TYPE_2_FRAME_1,
            {
                FIELD_CHECKSUM,
                FIELD_SYNC,
    ...
    trans = std::make_unique<TestTransceiver>(true);
    ASSERT_THROW(
        serial_library::SerialProcessor proc(std::move(trans), oneChecksum, Type2SerialFrames1::TYPE_2_FRAME_1, "a", 1),
        SerialLibraryException);
}
